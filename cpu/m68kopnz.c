...
void m68k_op_rte_32(void)
{
	...
	if(FLAG_S)
	{
		...
		{
        IDLE_DEBUG("RTE at x%06x",REG_PPC);

			new_sr = m68ki_pull_16();
			new_pc = m68ki_pull_32();
            IDLE_DEBUG("RTE new PC x%06x",new_pc);
            IDLE_DEBUG("RTE STACK PTR x%06x",m68k_get_reg(NULL,M68K_REG_A7));
			m68ki_jump(new_pc);
			m68ki_set_sr(new_sr);
...
{
    IDLE_INIT_FUNC("m68k_op_rts_32()");
	m68ki_trace_t0();				   /* auto-disable (see m68kcpu.h) */
    IDLE_DEBUG("RTS STACK PTR x%06x",m68k_get_reg(NULL,M68K_REG_A7));
	m68ki_jump(m68ki_pull_32());
}
