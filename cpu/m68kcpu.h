...
#define EXCEPTION_BUS_ERROR                2 /* This one is now emulated ;-) GFE 2007 */
...
#define RUN_MODE_AERR_RESET      1         // GFE differentiate bus and address error
#define RUN_MODE_BERR            2
...
#define REG_PPPC 		 m68ki_cpu.pppc
...
discard Bus error (already stacked and pc modified)
if we are in a real address error stack
*/

	#define m68ki_set_address_error_trap() \
		if(setjmp(m68ki_aerr_trap) != 0) \
		{ \
		    if (CPU_RUN_MODE == RUN_MODE_BERR) return m68ki_initial_cycles;\
	...
	#define m68ki_check_address_error(ADDR, WRITE_MODE, FC) \
		if (((ADDR)&1) && (CPU_RUN_MODE!=RUN_MODE_AERR_RESET)) \
		{ \
...
typedef struct
{
	...
	uint pppc;		   /* Previous Previous program counter */
...
INLINE uint m68ki_read_imm_16(void)
{
	...
	REG_PC += 2;
	if(MASK_OUT_BELOW_2(REG_PC-2) != CPU_PREF_ADDR)
	{
		CPU_PREF_ADDR = MASK_OUT_BELOW_2(REG_PC-2);
...
INLINE void m68ki_stack_frame_buserr(uint sr)
{
    ...
    case 0x4Ea8:
        stacked_pc=(REG_PPPC+2)&CPU_ADDRESS_MASK;
   	    ...
   	    if (sr&0x2000) {
             	m68ki_fake_pull_32();
                IDLE_DEBUG("correct SSP for RTS val=%06x",m68ki_cpu.dar[15]);
              }
       	else {
       	    m68ki_cpu.sp[0]+=4;
            IDLE_DEBUG("correct USP for RTS val=%06x",m68ki_cpu.sp[0]);
        }	  
	    break;
    ...
    m68ki_push_32(stacked_pc);IDLE_DEBUG("stacking PC x%08x",stacked_pc);
	...
	IDLE_DEBUG("stacking AERR x%08x",m68ki_aerr_address);
...
{
	uint sr = m68ki_init_exception();
    IDLE_INIT_FUNC("m68ki_exception_trapN()");

    IDLE_DEBUG("TRAPN at x%06x",REG_PPC);
    IDLE_DEBUG("TRAPN STACK PTR x%06x",m68k_get_reg(NULL,M68K_REG_A7));
...
{
	uint sr;
    IDLE_INIT_FUNC("m68ki_exception_address_error()");
...
INLINE void m68ki_exception_bus_error(void)
{
    ...
    IDLE_INIT_FUNC("m68ki_exception_bus_error()");
	 ...
	 * Halt the CPU
	 */
    IDLE_DEBUG("Offending instruction address x%06x",REG_PPC);
	if(CPU_RUN_MODE != RUN_MODE_NORMAL)
	{
        IDLE_TRACE("Double fault!"); 
		CPU_STOPPED = STOP_LEVEL_HALT;
...
// GFE : to avoid interrupts in exceptions
	/* Use up some clock cycles and undo the instruction's cycles */
	USE_CYCLES(CYC_EXCEPTION[EXCEPTION_ADDRESS_ERROR] - CYC_INSTRUCTION[REG_IR]);
	IDLE_DEBUG("Long jump");
...
void m68ki_exception_interrupt(uint int_level)
{
    ...
    IDLE_INIT_FUNC("m68ki_exception_interrupt()");
...
INLINE void m68k_tracenoirq(void) {
    IDLE_INIT_FUNC("m68k_tracenoirq()");
    IDLE_DEBUG("INT %x voided mask %x",CPU_INT_LEVEL,FLAG_INT_MASK);
}
